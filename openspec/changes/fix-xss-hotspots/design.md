...
**5. Discovered mid-implementation: reduce cognitive complexity in the 4 touched import functions.**
CI's SonarQube PR analysis failed the quality gate on 4 new CRITICAL `python:S3776` complexity violations — one per function touched by the blocker-bug fix (task group 3). This is pre-existing complexity (`import_survey` was already at 184 vs. 15 allowed), surfaced only because SonarQube attributes a function's whole complexity rating to "new code" whenever any line in it changes. Fixed via extract-method refactors: pull the per-row validation/object-construction logic into small private helper functions (`_build_dnc_contact_from_row`, `_build_contact_from_import_row` ×2 — kept separate per file rather than shared, since `admin.py`'s version has a subtly different behavior around `json.loads` error handling that must not change — and `_import_section_row`/`_import_branching_row`), leaving the outer view functions as thin loops that branch on the helper's return value. Every existing behavioral quirk is preserved exactly, including: `break` (not `continue`) stopping the whole loop on an invalid contact row, the precise scope of each `try/except` (e.g. `new_old_section[int(row[29])] = ...` staying inside the same try block as the `Section_template.objects.create()` call it follows), and `branching_row` never being populated (a pre-existing dead variable, left as-is).
