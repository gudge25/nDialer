...
- [x] 2.6 In SonarQube, mark the 12 corresponding hotspots (agent_detail.html:31/47, login.html:22, campaign/list.html:82/92, mod_sms/list.html:72/81/83, pagination.html:11/14/37/40) as reviewed/"Safe" with a short justification referencing this change; verify by reloading the project dashboard and confirming these no longer appear as TO_REVIEW/open. [Done via API. Also marked the 13th HIGH hotspot, survey_change.html:325 (the real fix's own call site), as reviewed/Safe with its own justification — Sonar can't see that the underlying filter now escapes the value, so the hotspot needed marking too despite the code fix.]
...
- [x] 5.5 Handle what remains after the refactor: fixed a genuinely-new `python:S5754` bare-`except:` (caught `ValueError` specifically — the moved line only ever raises that). The 5 still-over-threshold complexity findings (22-33, down from the originals but still >15) were left as-is per explicit user decision ("do not touch... Refactor this function to reduce its Cognitive Complexity") and resolved in SonarQube as won't-fix with justification, rather than pursued further into a larger, riskier restructuring of legacy import logic.

## 6. Verification

- [x] 6.1 Re-run the project's Python test suite and confirm no regressions, including the new survey branching-goto escaping test, the 4 CSV-import row-skip tests, and that the 4 refactored import functions still behave identically; verify via the test runner's pass/fail output (run for real via CI, not simulated). [151/151 passed on CI, PR #3.]
- [x] 6.2 Trigger a SonarQube re-analysis (via a real PR, as in the prior change) and confirm: 0 open HIGH-probability XSS hotspots, 0 BLOCKER bugs, 0 open MAJOR vulnerabilities, quality gate OK, 0 new violations; verify by comparing the new dashboard/PR analysis counts against the baseline captured in this change's proposal. [Confirmed on PR #3: quality gate OK, 0 open issues, 0 open hotspots, 151/151 tests passing. The 5 complexity findings are resolved as won't-fix (see 5.5), not eliminated — quality gate passes because they're resolved, not because the underlying complexity metric is under 15.]
